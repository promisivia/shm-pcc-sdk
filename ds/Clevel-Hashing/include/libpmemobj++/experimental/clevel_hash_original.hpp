...
class clevel_hash {
 public:
  ...
  using hasher = Hash;
  using key_equal = typename internal::key_equal_type<Hash, KeyEqual>::type;
  ...
  void foo() {
    ...
    std::cout << "sizeof(KV_entry_ptr_t) = " << sizeof(KV_entry_ptr_t)
              << std::endl;
  ...
  void set_thread_num(size_type num) {
    if (thread_num > 0) {
      ...
      delete_persistent<persistent_ptr<level_meta>[]>(tmp_meta, thread_num);
      delete_persistent<persistent_ptr<level_bucket>[]>(tmp_level, thread_num);
      delete_persistent<persistent_ptr<value_type>[]>(tmp_entry, thread_num);
    }
    ...
    tmp_level = make_persistent<persistent_ptr<level_bucket>[]>(thread_num);
    tmp_entry = make_persistent<persistent_ptr<value_type>[]>(thread_num);
  }

  // Only for debug use!
  KV_entry_ptr_t &get_entry(level_ptr_t level, difference_type idx,
                            uint64_t slot_idx);
  ...
  f_code_t find(pool_base &pop, const key_type &key, partial_t partial,
  ...
  f_code_t find_empty_slot(pool_base &pop, const key_type &key,
                           partial_t partial, size_type &n_levels,
                           KV_entry_ptr_t **e, uint64_t &level_num,
                           level_meta_ptr_t &m_copy);

  void expand(pool_base &pop, size_type thread_id, level_meta_ptr_t m_copy);
      ...
      for (size_type j = 0; j < assoc_num; j++) {
        if (f_b.slots[j].x.partial == partial &&
          ...
          if (key_equal{}(f_b.slots[j].p.get_address(my_pool_uuid)->first,
                          key)) {
            return ret(i, f_idx, j, f_b.slots[j].p.get_address(my_pool_uuid));
          }
        }
      }
      ...
      for (size_type j = 0; j < assoc_num; j++) {
        if (s_b.slots[j].x.partial == partial &&
          ...
          if (key_equal{}(s_b.slots[j].p.get_address(my_pool_uuid)->first,
                          key)) {
            return ret(i, s_idx, j, f_b.slots[j].p.get_address(my_pool_uuid));
          }
        }
      }
    ...
    *e = nullptr;
    ...
    for (size_type i = 0; i < n_levels; i++) {
      ...
      for (size_type j = 0; j < assoc_num; j++) {
        ...
        if (f_b.slots[j].x.partial != partial ||
            !key_equal{}(f_e.get_address(my_pool_uuid)->first, key))
          continue;
        ...
        if (result == FOUND_IN_LEFT || result == FOUND_IN_RIGHT) {
          ...
          if (f_e.get_offset() == prev_e.get_offset()) {
            ...
            // bottom level.
            if (prev_i < i) {
              del_dup(pop, &f_b.slots[j],
                      &(levels[level_num]
                            .get_address(my_pool_uuid)
                            ->buckets[idx]
                            .slots[slot_idx]),
                      f_e, prev_e);
            } else {
      ...
      for (size_type j = 0; j < assoc_num; j++) {
        ...
        if (s_b.slots[j].x.partial != partial ||
            !key_equal{}(s_e.get_address(my_pool_uuid)->first, key))
          continue;
...
clevel_hash<Key, T, Hash, KeyEqual, HashPower>::generic_insert(
  ...
  RETRY_INSERT:
    ...
    f_code_t result;
    if (check_duplicate) {
      result = find(pop, key, partial, n_levels, old_e, &e, level_num, idx,
                    /*fix_dup=*/false, thread_id, m_copy);
    } else {
      result =
          find_empty_slot(pop, key, partial, n_levels, &e, level_num, m_copy);
    }
...
clevel_hash<Key, T, Hash, KeyEqual, HashPower>::erase(const key_type &key,
  ...
  while (true) {
        ...
        if (tmp.x.partial == partial && tmp.p.get_offset() != 0) {
          if (key_equal{}(tmp.p.get_address(my_pool_uuid)->first, key)) {
            if (CAS(&(f_b.slots[j].p.off), tmp.p.off, 0)) {
              ...
              if (m_copy != meta || (i == 0 && f_idx <= expand_bucket &&
        ...
        if (tmp.x.partial == partial && tmp.p.get_offset() != 0) {
          if (key_equal{}(tmp.p.get_address(my_pool_uuid)->first, key)) {
            if (CAS(&(s_b.slots[j].p.off), tmp.p.off, 0)) {
              ...
              // before deletion's CAS. Therefore, we can do context
              // checking to avoid such failures.
              if (m_copy != meta || (i == 0 && s_idx <= expand_bucket &&
    ...
    // Context checking.
    if (m_copy == meta) return ret(succ_deletion);
  }  // end while(true)
}
...
clevel_hash<Key, T, Hash, KeyEqual, HashPower>::generic_update(
  ...
  while (true) {
    ...
    f_code_t result = find(pop, key, partial, n_levels, old_e, &e, level_num,
  ...
  if (cl->up == nullptr) {
    ...
    while (true) {
      if (cl->capacity >= new_capacity) {
        // Help updating meta
        make_persistent_atomic<level_meta>(pop, tmp_meta[t_id], m->first_level,
                                           m->last_level, true);
      } else {
        assert(cl->up != nullptr);
        make_persistent_atomic<level_meta>(pop, tmp_meta[t_id], cl->up,
                                           m->last_level, true);
      }
    ...
    if (meta == m_copy) {
      ...
      while (true) {
        // Help updating meta
        if (cl->capacity >= new_capacity) {
          make_persistent_atomic<level_meta>(
              pop, tmp_meta[t_id], m->first_level, m->last_level, true);
        } else {
          assert(cl->up != nullptr);
          make_persistent_atomic<level_meta>(pop, tmp_meta[t_id], cl->up,
                                             m->last_level, true);
        ...
        for (size_type j = 0; j < assoc_num; j++) {
          ...
          if (dst_tmp.get_offset() == 0) {
            if (CAS(&(dst_b1.slots[j].p.off), dst_tmp.raw(), src_tmp.raw())) {
              pop.persist(&(dst_b1.slots[j].p.off), sizeof(uint64_t));

              b.slots[slot_idx].p = nullptr;
              pop.persist(&(b.slots[slot_idx].p.off), sizeof(uint64_t));
              succ = true;
              break;
            }
          }
          ...
          if (dst_tmp.get_offset() == 0) {
            if (CAS(&(dst_b2.slots[j].p.off), dst_tmp.raw(), src_tmp.raw())) {
              pop.persist(&(dst_b2.slots[j].p.off), sizeof(uint64_t));
