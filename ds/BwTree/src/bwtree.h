...
class BwTreeBase {
 protected:
  ...
  static constexpr size_t cl_size = 64;

  // This is the mask we used for address alignment (AND with this)
  static constexpr size_t CACHE_LINE_MASK = ~(cl_size - 1);
...
protected:
...
#ifndef NT_SIM
  static_assert(sizeof(MetaData) < cl_size,
                "class Data size exceeds cache line length!");
#else
  static_assert(sizeof(MetaData) < 2*cl_size,
                "class Data size exceeds cache line length!");
#endif
  ...
  class GCMetaData {
   public:
    ...
    // be recycled
#ifdef NO_CC
    nt<uint64_t> last_active_epoch;
#else
    std::atomic<uint64_t> last_active_epoch;
#endif

#if defined(NO_CC) && defined(OPT_GC)
    // This is the minimum epoch when the value is updated
    nt<uint64_t> minimun_epoch;
#endif
...
#ifndef NT_SIM
  using PaddedMetadata = PaddedData<MetaData, cl_size>;
#else
  using PaddedMetadata = PaddedData<MetaData, 3 * cl_size>;
#endif

  static_assert(sizeof(PaddedMetadata) == PaddedMetadata::ALIGNMENT,
                "class PaddedMetadata size does"
 ...
 public:
  ...
  void PrepareThreadLocal() {
    ...
    gc_metadata_p = reinterpret_cast<PaddedGCMetadata *>(
        (reinterpret_cast<size_t>(original_p) + cl_size - 1) & CACHE_LINE_MASK);

    // Make sure it is aligned
    assert(((size_t)gc_metadata_p % cl_size) == 0);

    // Make sure we do not overflow the chunk of memory
    assert(((size_t)gc_metadata_p + thread_num * cl_size) <=
           ((size_t)original_p + (thread_num + 1) * cl_size));
    ...
    // Align the address to cache line boundary
    metadata_p = reinterpret_cast<PaddedMetadata *>(
        (reinterpret_cast<size_t>(original_p2) + cl_size - 1) & CACHE_LINE_MASK);

    // Make sure it is aligned
    assert(((size_t)metadata_p % cl_size) == 0);

    // Make sure we do not overflow the chunk of memory
    assert(((size_t)metadata_p + thread_num * cl_size) <=
           ((size_t)original_p2 + (thread_num + 1) * cl_size));
 ...
 public:
  ...
  inline void IncreaseEpoch() {
    ...
    for (int i = 0; i < static_cast<int>(thread_num); i++) {
      ...
      min_epoch = std::min(meta->last_active_epoch.load(), min_epoch);
      GetGCMetaData(i)->last_active_epoch.store(epoch);
    }

    // update all thread's minimun_epoch
    for (int i = 0; i < static_cast<int>(thread_num); i++) {
      auto meta = GetGCMetaData(i);
      meta->minimun_epoch.store(min_epoch);
    }
#endif
  ...
  /*
   * GetMetaData() - Returns the thread-local metadata for GC for a specified
   *                   thread
   */
  inline MetaData *GetMetaData(int thread_id) {
    // The thread ID must be within the range
    // assert(thread_id >= 0 && thread_id < static_cast<int>(thread_num));

    return &(metadata_p + thread_id)->data;
  }

  /*
   * GetCurrentMetaData() - Returns the metadata for the current thread
   */
  inline MetaData *GetCurrentMetaData() { return GetMetaData(gc_id); }
  ...
  uint64_t SummarizeGCEpoch() {
    ...
    uint64_t min_epoch = GetGCMetaData(0)->last_active_epoch.load();

    // This might not be executed if there is only one thread
    for (int i = 1; i < static_cast<int>(thread_num); i++) {
      // Note: std::min pass a const & of into the function. We need to first
      // copy the shared GetGCMetaData(i)->last_active_epoch into a local
      // variable before calling std::min. Otherwise we will have a Heisenbug
      // where std::min first check which one is smaller, and before it returns,
      // other thread modify the variable and we actually return the larger one.
      auto ts = GetGCMetaData(i)->last_active_epoch.load();
...
#if defined(NO_CC) && defined(OPT_GC)
  uint64_t SummarizeGCEpoch2(int thread_id) {
    return GetGCMetaData(thread_id)->minimun_epoch.load();
  }
#endif
};
...
class BwTree : public BwTreeBase {
 ...
 public:
  ...
  class BaseNode {
   ...
   public:
    virtual void FlushNode() const { clwb(this, sizeof(BaseNode)); }
    /*
    ...
    /*
     * SetHighKeyPair() - Sets the high key pair of metadata
     */
  ...
  class LeafDataNode : public DeltaNode {
   public:
    ...
    void FlushNode() const override {
      clwb(this, sizeof(LeafDataNode));
    }
  ...
  class LeafInsertNode : public LeafDataNode {
   public:
    void FlushNode() const override {
      clwb(this, sizeof(LeafInsertNode));
    }
    ...
    LeafInsertNode(const KeyType &p_insert_key, const ValueType &p_value,
        ...
        : LeafDataNode{
              ...
              // For insert nodes, the item count is inheried from
              // the child node + 1 since it inserts new item
  ...
  class LeafDeleteNode : public LeafDataNode {
   public:
    void FlushNode() const override {
      clwb(this, sizeof(LeafDeleteNode));
    }
    /*
     * Constructor
     */
    LeafDeleteNode(const KeyType &p_delete_key, const ValueType &p_value,
        ...
        : LeafDataNode{
              ...
              p_child_node_p, p_index_pair, &p_child_node_p->GetLowKeyPair(),
              &p_child_node_p->GetHighKeyPair(), p_child_node_p->GetDepth() + 1,
              // For delete node it inherits item count from its
              // child and - 1 from it since one element was deleted
              p_child_node_p->GetItemCount() - 1} {}
  };
  ...
  class LeafSplitNode : public DeltaNode {
   public:
    ...
    void FlushNode() const override {
      clwb(this, sizeof(LeafSplitNode));
    }

    /*
     * Constructor
     *
     * NOTE: The constructor requires that the physical pointer to the
     * split sibling being passed as an argument. It will not be stored
     * inside the split delta, but it will be used to compute the new
     * item count for the current node
     */
    LeafSplitNode(const KeyNodeIDPair &p_insert_item,
        ...
        : DeltaNode{NodeType::LeafSplitType, p_child_node_p,
                    ...
                    // High key is redirected to the split item inside
                    // the node
                    &insert_item,
                    ...
                    // For split node it is a little bit tricky - we
                    // must know the item count of its sibling to decide
                    // how many items were removed by the split delta
   ...
   * EpochManager to recycle NodeID correctly, since when a
   * LeafRemoveNode is being recycled, the removed NodeID is also
   * recycled
   */
  class LeafRemoveNode : public DeltaNode {
   public:
    ...
    void FlushNode() const override {
      clwb(this, sizeof(LeafRemoveNode));
    }
   ...
   *
   * This structure uses two physical pointers to indicate that the
   * right half has become part of the current node and there is no
  ...
  class LeafMergeNode : public DeltaNode {
   public:
    ...
    void FlushNode() const override {
      clwb(this, sizeof(LeafMergeNode));
    }
  ...
  /*
   * class InnerDataNode - Base class for InnerInsertNode and
   * InnerDeleteNode
   *
   * We need this node since we want to sort pointers to such nodes
  ...
  class InnerDataNode : public DeltaNode {
   public:
    ...
    // search key is smaller than this key then binary search could end
    // before this pointer
    const KeyNodeIDPair *location;

    void FlushNode() const override {
      clwb(this, sizeof(InnerDataNode));
    }
  ...
  class InnerInsertNode : public InnerDataNode {
   public:
    ...
    void FlushNode() const override {
      clwb(this, sizeof(InnerInsertNode));
    }
  ...
  class InnerDeleteNode : public InnerDataNode {
   public:
    ...
    // should be a constant and kept inside each node on the delta
    // chain) then do not need to compare to it since the search key
    // must >= low key But if the prev_item.second != low key node id
    // then need to compare key
    KeyNodeIDPair prev_item;

    // This holds the next key-NodeID item in the inner node
    // If the NodeID inside next_item is INVALID_NODE_ID then we do not
    // have to conduct any comparison since we know it is the high key
    KeyNodeIDPair next_item;

    void FlushNode() const override {
      clwb(this, sizeof(InnerDeleteNode));
    }
     ...
     * it does make sense for tree traversal. But when the tree
     * destructor runs it needs the deleted NodeID information in order
     * to avoid traversing to a node that has already been deleted and
     * been recycled
     */
  ...
  class InnerSplitNode : public DeltaNode {
   public:
    ...
    void FlushNode() const override {
      clwb(this, sizeof(InnerSplitNode));
    }
    ...
    InnerSplitNode(const KeyNodeIDPair &p_insert_item,
        ...
        : DeltaNode{NodeType::InnerSplitType, p_child_node_p,
                    ...
                    // For split node depth does not change since it
                    // does not introduce new data
                    p_child_node_p->GetDepth(),
                    // For split node we need the physical pointer to
                    // the split sibling to compute item count
                    p_child_node_p->GetItemCount() -
                        p_split_node_p->GetItemCount()},
          insert_item{p_insert_item} {}
  };
  ...
  class InnerRemoveNode : public DeltaNode {
   public:
    ...
    void FlushNode() const override {
      clwb(this, sizeof(InnerRemoveNode));
    }
  ...
  class InnerMergeNode : public DeltaNode {
   public:
    ...
    void FlushNode() const override {
      clwb(this, sizeof(InnerMergeNode));
    }
    ...
    InnerMergeNode(const KeyType &p_merge_key, const BaseNode *p_right_merge_p,
        ...
        : DeltaNode{NodeType::InnerMergeType, p_child_node_p,
                    &p_child_node_p->GetLowKeyPair(),
                    &p_right_merge_p->GetHighKeyPair(),
                    // Note: Since both children under merge node is
                    // considered as part of the same node, we use the
                    // larger one + 1 as the depth of the merge node
                    // std::max(p_child_node_p->metadata.depth,
                    //         p_right_merge_p->metadata.depth) + 1,
                    p_child_node_p->GetDepth() + p_right_merge_p->GetDepth(),
                    // For merge node the item count is the sum of its
                    // two branches
                    p_child_node_p->GetItemCount() +
                        p_right_merge_p->GetItemCount()},
          delete_item{p_merge_key, p_deleted_node_id},
  ...
  class InnerAbortNode : public DeltaNode {
   public:
    void FlushNode() const override {
      clwb(this, sizeof(InnerAbortNode));
    }
  ...
  /*
   * struct NodeSnapshot - Describes the states in a tree when we see
   * them
   *
   * node_id and node_p are pairs that represents the state when we
   * traverse the node and use GetNode() to resolve the node ID.
   */
  class NodeSnapshot {
   public:
    NodeID node_id;
     ...
     *
     * This function is just a wrapper of IsOnLeafDeltaChain() in
     * BaseNode
     */
  ...
  /*
   * union DeltaNodeUnion - The union of all delta nodes - we use this
   * to precllocate memory on the base node for delta nodes
   */
  ...
  class AllocationMeta {
   ...
   public:
     ...
     *
     * Note that this function retries internally if CAS fails.
     * Therefore, the number of CAS loop is upperbounded by the chunk
     * size
     */
    void *TryAllocate(size_t size) {
      // This acts as a guard to prevent allocating from a chunk which
      // has already underflown to avoid integer (64 bit pointer)
      // underflow
      if (tail.load() < limit) {
        return nullptr;
      }
...
#ifdef NO_CC
      char *new_tail = tail.fetch_sub((char*)size) - size;
#else
     ...
     *
     * This procedure is thread safe since we always CAS the next
     * pointer with expected value nullptr, such that it should never
     * succeed twice even under contention
     *
     * Whether or not this has succeded, always return the pointer to
     * the next chunk such that the caller could retry on next chunk
     */
...
#ifdef USE_CXL
        cacheable.free(reinterpret_cast<char *>(meta_p));
#else
  ...
  class ElasticNode : public BaseNode {
   private:
    ...
    std::array<nt<uint64_t>, OPT_DUP_IN_USE_FLAG> status_flag;
#else
    nt<uint64_t> status_flag;
#endif
#endif
...
// #ifdef OPT_IN_USE_FLAG
...
#else
      return status_flag.load() == NOT_IN_USE;
#endif
    }

    // bool IsUpdated() const {
    //   return status_flag.load() == UPDATED;
    // }

    void SetNotInUse() {
#ifdef OPT_DUP_IN_USE_FLAG
      for (int i = 0; i < OPT_DUP_IN_USE_FLAG; i++) {
        status_flag[i].store(NOT_IN_USE);
      }
#else
      status_flag.store(NOT_IN_USE);
#endif
    }

#ifdef OPT_PASS_CHAIN_HEAD_WITH_FLAG
    void SetChainHead(BaseNode *chain_head) {
#ifdef OPT_DUP_IN_USE_FLAG
      for (int i = 0; i < OPT_DUP_IN_USE_FLAG; i++) {
        status_flag[i].store((uintptr_t)chain_head);
      }
#else
      status_flag.store((uintptr_t)chain_head);
#endif
    }

    BaseNode *GetChainHead() const {
#ifdef OPT_DUP_IN_USE_FLAG
      return (BaseNode*) status_flag[gc_id % OPT_DUP_IN_USE_FLAG].load();
#else
      return (BaseNode*) status_flag.load();
#endif
    }
#endif

    // void SetUpdated() {
    //   status_flag.store(UPDATED);
    // }
#endif
    ...
    void FlushNode() const override {
      clwb(
          this, sizeof(ElasticNode) + sizeof(ElementType) * (End() - Begin()));
    }
...
#ifdef OPT_ROOT_READ
    size_t node_count = FreeNodeByNodeID(global_root_id.load());
#else
...
#ifdef OPT_ROOT_READ
    global_root_id.store(GetNextNodeID());
    assert(global_root_id.load() == 1UL);
#else
    root_id = GetNextNodeID();
    assert(root_id.load() == 1UL);
#endif
...
#ifdef OPT_ROOT_READ
    int _root_id = global_root_id.load();
#else
    int _root_id = root_id.load();
#endif
    bwt_printf("root id = %lu; first leaf id = %lu\n", _root_id,
               first_leaf_id);

    InstallNewNode(_root_id, root_node_p);
...
// count to be 0
  ...
  inline uint64_t GetRootID(bool bypass_cache = BYPASS_CACHE_DEFAULT) {
#ifdef OPT_ROOT_READ
    // bypass_cache decides how we load cached_root_id. We can't load
    // global_root_id here.
...
#else
    return root_id.load(std::memory_order_seq_cst);
#endif
  }
...
// #else
    // bool bypass_cache = BYPASS_CACHE_DEFAULT;
// #endif
  ...
  retry_traverse:
    ...
    NodeID start_node_id = GetRootID();
    assert(start_node_id != INVALID_NODE_ID);
...
// #if defined(NO_CC) && defined(OPT_IN_USE_FLAG)
//       if (context_p->need_retry_slow_path) {
...
// #endif
  ...
  // retry_slow_path_traverse:
  //   context_p->need_retry_slow_path = false;
...
// #endif
   ...
   * similar problems since now the snapshot has been changed, and we need to
   * check whether there is data in the map again to make sure there is no map
   * insert conflict
   */
...
// #endif
...
#ifdef OPT_IN_USE_FLAG
    if (parent_snapshot_p->node_p != GetNode(parent_snapshot_p->node_id, context_p->retrying_slow_path)) {
#else
  ...
  void LoadNodeID(NodeID node_id, Context *context_p,
    ...
    TakeNodeSnapshot(node_id, context_p, bypass_cache);
  ...
  inline void TakeNodeSnapshotReadOptimized(NodeID node_id,
    ...
    // TO RIGHT SIBLING SINCE IT CHECKS NODE ID
    context_p->current_snapshot.node_id = node_id;
...
// #ifdef OPT_IN_USE_FLAG
//       NodeSnapshot halfway_parent_snapshot = context_p->parent_snapshot;
//       NodeSnapshot halfway_current_snapshot = context_p->current_snapshot;
#ifdef OPT_PASS_CHAIN_HEAD_WITH_FLAG
    retry_half_slow_path_traverse:
#endif
      // #endif
      LoadNodeIDReadOptimized(child_node_id, context_p);
...
// #endif
  ...
  inline bool PostInnerInsertNode(Context *context_p,
    ...
    // whether to consolidate and/or split the node
    NodeSnapshot *parent_snapshot_p = GetLatestParentNodeSnapshot(context_p);
   ...
   * help-along protocol. It posts an InnerDeleteNode on the parent node and
   * returns the result of CAS instruction
   *
   * NOTE: This function may abort
   */
  ...
  void FinishPartialSMO(Context *context_p) {
    ...
    // must also be updated
    NodeSnapshot *snapshot_p = GetLatestNodeSnapshot(context_p);
    bwt_printf("snapshot_p->node_p=%p\n", snapshot_p->node_p);
...
// constructor
          ...
          // term and the split delta has not been consolidated yet, so that a
          // thread sees the split delta and traverses to its right sibling
          // which is on another parent node (the right sibling of the current
          // parent node) In this case we do not have to insert since we know
          // the index term has already been inserted
          ...
          const KeyNodeIDPair *found_item_p = NavigateInnerNode(
              parent_snapshot_p, insert_item_p->first, &location, context_p);
          ...
          // SMO which means the SMO could be removed by a consolidation to
          // avoid consolidating the parent node again and again Note: Even if
          // this function aborts, since we return immediately so do not have to
          // test abort_flag here
...
// #endif
  ...
  void ConsolidateNode(NodeSnapshot *snapshot_p) {
    // printf("ConsolidateNode node_p: %p\n", snapshot_p->node_p);
...
#ifdef OPT_ROOT_READ
        /* read optimization might direct a old root */
        uint64_t tmp_root_id = global_root_id.load();
        if (GetRootID() != tmp_root_id) {
          GetCurrentMetaData()->cached_root_id.store(tmp_root_id);
          GetCurrentMetaData()->cached_root_node_p.store(
  ...
  const KeyNodeIDPair *NavigateInnerNode(NodeSnapshot *snapshot_p,
                                         const KeyType &search_key,
                                         const KeyNodeIDPair **location,
                                         const Context *context_p = nullptr) {
    // Save some keystrokes
    const BaseNode *node_p = snapshot_p->node_p;
    ...
    while (1) {
      switch (node_p->GetType()) {
        ...
        default: {
          printf("Unknown InnerNode type: %d, node_p=%p", (int)node_p->GetType(), node_p);
...
#ifdef OPT_PASS_CHAIN_HEAD_WITH_FLAG
  void MarkLeafNodeChainHeadRecursive(const BaseNode *node,
                                      const BaseNode *chain_head) {
    NodeType type;
    while (1) {
      type = node->GetType();
      switch (type) {
        case NodeType::LeafType:
          const_cast<LeafNode *>(static_cast<const LeafNode *>(node))
              ->SetChainHead(const_cast<BaseNode*>(chain_head));
          return;
        case NodeType::LeafInsertType:
          node = static_cast<const LeafInsertNode *>(node)->child_node_p;
          break;
  ...
  bool Insert(const KeyType &key, const ValueType &value) {
    ...
    while (1) {
      ...
      // printf("Allocate LeafInsertNode id:%d p:%p\n", node_id, insert_node_p);
...
retry_read:
...
#ifdef OPT_IN_USE_FLAG
    if (value_list.size() == 0 && !context.retrying_slow_path) [[unlikely]] {
#ifdef BWTREE_RETRY_COUNT
      full_counter->Increment();
#endif
      context.current_snapshot.node_id = INVALID_NODE_ID;
      context.retrying_slow_path = true;
      goto retry_read;
    }
#endif
 ...
 public:
  ...
  void reportTimers() {
#ifdef BWTREE_TIMING_ROOT_LOAD
    std::cout << "Report timer: " << std::endl;
    read_root_timer.printHistogram();
#endif
  }
