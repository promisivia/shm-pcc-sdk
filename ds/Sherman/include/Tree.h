...
public:
  ...
  void insert(const ShermanKey &k, const Value &v, CoroContext *cxt = nullptr,
              int coro_id = 0);
  bool search(const ShermanKey &k, Value &v, CoroContext *cxt = nullptr,
              int coro_id = 0);
  void del(const ShermanKey &k, CoroContext *cxt = nullptr, int coro_id = 0);

  uint64_t range_query(const ShermanKey &from, const ShermanKey &to, Value *buffer,
  ...
  void lock_bench(const ShermanKey &k, CoroContext *cxt = nullptr, int coro_id = 0);
...
private:
  ...
  bool update_new_root(GlobalAddress left, const ShermanKey &k, GlobalAddress right,
                       int level, GlobalAddress old_root, CoroContext *cxt,
                       int coro_id);

  void insert_internal(const ShermanKey &k, GlobalAddress v, CoroContext *cxt,
  ...
  bool page_search(GlobalAddress page_addr, const ShermanKey &k, SearchResult &result,
                   CoroContext *cxt, int coro_id, bool from_cache = false);
  void internal_page_search(InternalPage *page, const ShermanKey &k,
                            SearchResult &result);
  void leaf_page_search(LeafPage *page, const ShermanKey &k, SearchResult &result);

  void internal_page_store(GlobalAddress page_addr, const ShermanKey &k,
                           GlobalAddress value, GlobalAddress root, int level,
                           CoroContext *cxt, int coro_id);
  bool leaf_page_store(GlobalAddress page_addr, const ShermanKey &k, const Value &v,
                       GlobalAddress root, int level, CoroContext *cxt,
                       int coro_id, bool from_cache = false);
  bool leaf_page_del(GlobalAddress page_addr, const ShermanKey &k, int level,
                     CoroContext *cxt, int coro_id, bool from_cache = false);

  bool acquire_local_lock(GlobalAddress lock_addr, CoroContext *cxt,
                          int coro_id);
  bool can_hand_over(GlobalAddress lock_addr);
  void release_local_lock(GlobalAddress lock_addr);
};
...
public:
  ...
  InternalPage(GlobalAddress left, const ShermanKey &key, GlobalAddress right,
               uint32_t level = 0) {
