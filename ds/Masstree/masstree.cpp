...
leafvalue *masstree::make_leaf(char *key, size_t key_len, uint64_t value)
{
    ...
    if (value != 0)
        clwb((char *) lv, sizeof(leafvalue) + len + sizeof(uint64_t));
    return lv;
}
...
leafvalue *leafnode::smallest_leaf(size_t key_len, uint64_t value)
{
    ...
    if (value != 0)
        clwb((char *) lv, sizeof(leafvalue) + len);
    return lv;
}
        ...
        leafvalue *nlv, uint32_t depth)
{
    ...
    if (twig_head != l) {
        ...
        for ( ; iter != twig_tail && iter != NULL; iter = reinterpret_cast <leafnode *>(iter->entry[0].value)) {
            clwb((char *)iter, sizeof(leafnode));
        }
        clwb((char *)twig_tail, sizeof(leafnode));
        mfence();

        l->entry[kx_.p].value = twig_head;
        clwb((char *)l->entry_addr(kx_.p) + 8, sizeof(uintptr_t));
    } else {
        clwb((char *)nl, sizeof(leafnode));
...
void leafnode::check_for_recovery(masstree *t, leafnode *left, leafnode *right, void *root, uint32_t depth, leafvalue *lv)
{
    ...
    if (depth > 0) {
        ...
        if (p->value(pkx_.p) == left) {
            ...
            clwb((char *) &p->entry[pkx_.p].value, sizeof(uintptr_t));
            p->writeUnlock(false);
...
leaf_retry:
    ...
    if (kx_.p >= 0) {
        ...
        // ii)  Atomically update value for the matching key
            ...
            clwb((char *)&(LV_PTR(l->value(kx_.p)))->value, sizeof(void *));
            l->writeUnlock(false);
...
{
    entry[p].value = value;
    clwb((char *)&entry[p].value, sizeof(void *));
}
...
#ifdef NO_CC
leafnode *leafnode::search_for_leftsibling(nt<void*>* aroot, void **root, uint64_t key, uint32_t level, leafnode *right)
#else
...
void *leafnode::leaf_insert(masstree *t, void *root, uint32_t depth, leafvalue *lv, uint64_t key, void *value, key_indexed_position &kx_)
{
    ...
    if (this->permutation.size() < LEAF_WIDTH) {
        ...
        clwb((char *)(&this->entry[kx_.p]), sizeof(kv));
        ...
        if (split_type == 0) {
            ...
            clwb((char *)(&nl->entry[kx_.p]), sizeof(kv));
        ...
        if (depth > 0) {
            ...
            if (p->value(pkx_.p) == nr) {
                ...
                if (merge_state == 16) {
                    ...
                    clwb((char *)&p->entry[pkx_.p].value, sizeof(void *));
                    p->writeUnlock(false);
                }
            }
        } else {
        ...
        uint64_t key, void *value, key_indexed_position &kx_, leafnode *child, bool child_isOverWrite)
{
    ...
    if (this->permutation.size() < LEAF_WIDTH) {
        ...
        if (key < split_key) {
            ...
            clwb((char *)(&nr->entry[kx_.p]), sizeof(kv));
