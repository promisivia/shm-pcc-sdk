...
class permuter {
    public:
        ...
        void exchange(int i, int j) {
            ...
            x_.fetch_xor( (diff << (i << 2)) | (diff << (j << 2)));
        }
...
#ifdef NO_CC
        nt<leafnode *> next;                          // 8bytes
        nt<uint64_t> typeVersionLockObsolete{0b100};  // 8bytes
#else
...
#ifdef NO_CC
        leafnode *search_for_leftsibling(nt<void*> *root1, void **root, uint64_t key, uint32_t level, leafnode *right);
#else
